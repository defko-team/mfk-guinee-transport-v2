...
class AuthService {
  ...
  Future<String?> sendOtp(String phoneNumber) async {
    try {
      ...
      await _auth.verifyPhoneNumber(
        ...
        verificationFailed: (FirebaseAuthException e) {
          completer.completeError(
            OtpVerificationException('Échec de la vérification OTP: ${e.message}')
          );
        },
      ...
      throw OtpVerificationException('Impossible d\'obtenir l\'ID de vérification');
    }
  }
    ...
    try {
        ...
        try {
            ...
            if (authError.code == 'session-expired') {
                throw Exception('Le code OTP a expiré. Veuillez demander un nouveau code.');
            } else if (authError.code == 'invalid-verification-code') {
                throw Exception('Le code OTP est invalide. Veuillez vérifier et réessayer.');
            }
        ...
        if (user == null) {
            throw Exception('Échec de l\'authentification: aucun utilisateur trouvé');
        }
        ...
        if (isRegistration) {
            ...
            if (isDriver) {
                ...
                await _firestore.collection('Users').doc(newUserId).set(userModel.toMap());
        ...
        if (e.code == 'session-expired') {
            throw Exception('Le code OTP a expiré. Un nouveau code va vous être envoyé.');
        }
...
Future<void> _storeUserInPreferences(String userId) async {
    ...
    try {
        ...
        DocumentSnapshot roleDoc = await _firestore.collection('roles').doc(userData['id_role']).get();
...
Future<void> _redirectUserBasedOnRole(String userId, BuildContext context) async {
